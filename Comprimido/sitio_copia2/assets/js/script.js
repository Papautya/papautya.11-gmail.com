...
const API_URL = `${BASE_URL}/discover/movie?language=es-ES&sort_by=popularity.desc&${API_KEY}&certification_country=US&certification.lte=PG&with_genres=16`;
const IMG_URL = "https://image.tmdb.org/t/p/w500";
const searchURL = `${BASE_URL}/search/movie?${API_KEY}&certification_country=US&certification.lte=PG&with_genres=16`;
...
async function showMovies(data) {
  ...
  for (const movie of data) {
    ...
    const imageUrl = poster_path
      ? IMG_URL + poster_path
      : "../img/no_image.jpg"; // Ruta de la imagen predeterminada
    ...
    try {
      const response = await fetch(
        BASE_URL + "/movie/" + id + "/credits?" + API_KEY + "&language=es-ES"
      );
      const creditsData = await response.json();

      if (creditsData) {
        names += creditsData.cast
          .slice(0, 5)
